...
def assert_common(self, response, http_code, success, code, message):
    ...
    self.assertIn(message, response.json().get("message"))  # 断言mess
...
def read_add_emp_data():
    ...
    with open(path, mode='r', encoding='utf-8') as f:
        jsonData = json.load(f)
        # employee.json是一个字典数据，可以使用字典的get方法获取
...
class DBUtils:
    ...
    def __init__(self, host="182.92.81.159", user="readuser", password="'iHRM_user_2019", databas="ihrm"):
        self.host = host
        self.user = user
